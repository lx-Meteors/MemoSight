...
def _update_cache(
    ...
    """Update a regular Transformers cache or the token-aware SepLLM cache."""
    if isinstance(past_key_value, SepCache):
        if input_ids is None:
            raise ValueError("`input_ids` must be provided when using SepCache.")
        return past_key_value.update(
            ...
            PREFILLING_FLAG=key_states.shape[-2] > 1,
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            key_states, value_states = _update_cache(
                ...
                cache_kwargs, input_ids, position_ids,
            )
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        # SepCache stores fewer KV entries than the number of tokens seen. Build
        # the mask for the physical cache length while keeping absolute RoPE
        # positions in ``cache_position``.
        if isinstance(past_key_values, SepCache):
            target_length = past_key_values.get_usable_length(0) + sequence_length
        # SlidingWindowCache or StaticCache
        elif using_sliding_window_cache or using_static_cache:

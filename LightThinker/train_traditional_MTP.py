...
from model_qwen_traditional_MTP import Qwen2ForCausalLM
...
_MTP_PARAM_PREFIXES = ("mtp_modules.", "mtp_norm.")


def count_mtp_extra_parameters(model: torch.nn.Module) -> Tuple[int, int, List[Tuple[str, int, bool]]]:
    """
    统计 Qwen2ForCausalLM（traditional MTP）中 MTP 专属子模块参数量。
    仅包含 mtp_modules / mtp_norm；主干的 lm_head 与 MTP 头共享，不计入「额外」参数。
    """
    total = 0
    trainable = 0
    details: List[Tuple[str, int, bool]] = []
    for name, p in model.named_parameters():
        if not name.startswith(_MTP_PARAM_PREFIXES):
            continue
        n = p.numel()
        total += n
        if p.requires_grad:
            trainable += n
        details.append((name, n, p.requires_grad))
    return total, trainable, details
...
def print_mtp_parameter_report(model: torch.nn.Module) -> None:
    mtp_total, mtp_trainable, details = count_mtp_extra_parameters(model)
    all_total, all_trainable = count_model_parameters(model)
    print("\n========== MTP模块额外参数统计 ==========")
    if mtp_total == 0:
        print("未检测到 mtp_modules / mtp_norm（可能 mtp_depth=0 或非 Qwen traditional MTP 结构）。")
        print(f"全模型参数量: {all_total:,}（可训练 {all_trainable:,}）")
        print("==========================================\n")
        return
    pct = 100.0 * mtp_total / all_total if all_total else 0.0
    pct_tr = 100.0 * mtp_trainable / all_trainable if all_trainable else 0.0
    print(f"MTP 额外参数总量: {mtp_total:,}（可训练 {mtp_trainable:,}）")
    print(f"全模型参数量: {all_total:,}（可训练 {all_trainable:,}）")
    print(f"MTP 占全模型比例: {pct:.4f}%（按可训练计: {pct_tr:.4f}%）")
    print("--- 各张量明细（名称 / 元素数 / 是否训练） ---")
    for name, n, req in details:
        print(f"  {name}: {n:,}, requires_grad={req}")
    print("==========================================\n")


def _fill_defaults_for_print_mtp_only(args: argparse.Namespace) -> None:
    """仅打印参数模式：为未提供的训练相关 CLI 填占位值，避免 None传入后续逻辑。"""
    if args.tokenizer_path is None:
        args.tokenizer_path = args.model_path
    args.train_path = args.train_path or ""
    args.output_dir = args.output_dir or "/tmp/mtp_params_print_only"
    args.epochs = args.epochs if args.epochs is not None else 1
    args.lr = args.lr if args.lr is not None else 1e-5
    args.save_steps = args.save_steps if args.save_steps is not None else 1
    args.deepspeed = args.deepspeed or ""
    args.micro_batch_size = args.micro_batch_size if args.micro_batch_size is not None else 1
    args.gradient_accumulation_steps = (
        args.gradient_accumulation_steps if args.gradient_accumulation_steps is not None else 1
    )
    args.mode = args.mode or "normal"
    args.qkv = args.qkv if args.qkv is not None else ""
    args.output_compress_instruction = args.output_compress_instruction or "None"
    for name, default in (
        ("see_current", False),
...
def get_parser():
    ...
    parser.add_argument(
        '--print_mtp_params_only',
        action='store_true',
        help='仅构建模型并打印 MTP 额外参数统计，然后退出（不加载数据、不训练）',
    )
...
def get_model_and_tokenizer(
    ...
    print_mtp_parameter_report(model)
...
def main():
    args = get_parser()
    if args.print_mtp_params_only:
        _fill_defaults_for_print_mtp_only(args)
        if not args.model_path or not args.compress_config:
            print("错误: --print_mtp_params_only 需要同时提供 --model_path 与 --compress_config", file=sys.stderr)
            sys.exit(1)

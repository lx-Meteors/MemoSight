...
class Tokenizer:
    ...
    def get_separator_token_ids(self) -> List[int]:
        """Resolve SepLLM separator IDs for the active tokenizer instead of hard-coding Qwen2 IDs."""
        separator_texts = (".", ",", "?", "!", ";", ":", " ", "\t", "\n")
        token_ids = []
        for text in separator_texts:
            token_ids.extend(self.tokenizer.encode(text, add_special_tokens=False))
        # Preserve tokenizer order while dropping duplicate IDs.
        return list(dict.fromkeys(int(token_id) for token_id in token_ids))
